    ...
    callback: function (res) {
      if (res.message && res.message.length > 0) {
        ...
        data.forEach(function (item) {
          ...
          const accountTd = $("<td>").appendTo(row);
          const accountSelect = $("<select>", {
            class: "form-control",
            required: true,
          }).appendTo(accountTd);

          $("<option>", { value: "", text: "-- Select Account --" }).appendTo(
            accountSelect,
          );

          // Fetch accounts from default company
          frappe.call({
            method: "frappe.client.get_list",
            args: {
              doctype: "Account",
              ...
              if (r.message) {
                r.message.forEach(function (acc) {
                  const opt = $("<option>", {
                    value: acc.name,
                    text: acc.name,
                  }).appendTo(accountSelect);

                  // Pre-select if item already has a saved account
                  if (item.account && item.account === acc.name) {
          ...
          const addButton = $("<button>", {
            ...
            click: function () {
              if (!accountSelect.val()) {
                frappe.msgprint(__("Please select an Account before adding."));
                return;
              }
              const rowData = {
                ...
                tax_applicable_based_on_regime:
                  item.tax_applicable_based_on_regime,
                regime: item.regime,
                ...
                custom_field = [
                  {
                    ...
                    field_type: "Check",
                    field_name:
                      rowData.salary_component
                        .toLowerCase()
                        .replace(/[^a-z]/g, "") +
                      rowData.abbr.toLowerCase().replace(/[^a-z]/g, ""),
                    depends: "",
              ...
              function callCreateComponentAPI() {
                frappe.call({
                  method:
                    "cn_indian_payroll.cn_indian_payroll.overrides.payroll_configuration.get_salary_component",
                  args: {
                    data: rowData,
                  ...
                  callback: function (response) {
                    if (response && response.exc) {
                      frappe.msgprint(
                        __("Server returned an error: ") + response.exc,
                      );
                    } else {
                      frappe.msgprint(__("Component added successfully."));

                      if (
                        rowData.visibility_type == "Fixed" &&
                        rowData.multi_insert == 0
                      ) {
                  ...
                  error: function (err) {
                    ...
                    frappe.msgprint(
                      __("Failed to add component. Please check console."),
                    );
                  },
                });
              }
              ...
              if (rowData.multi_insert !== 1) {
                frappe.call({
                  ...
                  callback: function (current_res) {
                    if (
                      current_res.message &&
                      Array.isArray(current_res.message.custom_field_child)
                    ) {
                      $.each(
                        current_res.message.custom_field_child,
                        function (i, v) {
